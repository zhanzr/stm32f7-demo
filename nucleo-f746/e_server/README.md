# e_server - embedded web demo for the NUCLEO-F746ZG (frontend + reference C backend)

A minimal single-page web app for the NUCLEO-F746ZG (LED tab wired to the
board's PB0/PB7/PB14 LEDs, sensor tab to the ADC internal channels), plus a host-side, plus a host-side
...
1. **LED control** - three checkboxes reflecting the board LEDs (LD1 PB0
   green, LD2 PB7 blue, LD3 PB14 red). Each click
...
[ style.css  ] ──�?                                         (raw)        �?[ app.js     ] ──�?                                                         �?[ public/*.jpg ] ─────────────────────────�?raw byte arrays + embedded_files[]  web_assets.h
```
...
for an embedded server. **Already done** in `disco-f769/bare/eth_http/` (disco)
and `nucleo-f746/bare/eth_http_server/` (static IP): each project's
