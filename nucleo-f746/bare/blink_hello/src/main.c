 ...
 * Factory calibration on the F746 (stm32f746xx.h, acquired at Vref+ = 3.3 V):
 *  - VREFINT_CAL_ADDR_CMSIS        => 0x1FF0F44A
 *  - TEMPSENSOR_CAL1_ADDR_CMSIS    => 0x1FF0F44C (30  degC)
 *  - TEMPSENSOR_CAL2_ADDR_CMSIS    => 0x1FF0F44E (110 degC)
 *
 * Note: the F746 has an SFPU (single-precision FPU only); the board layer
...
static void ADC_Print(void)
{
    ...
    /* All arithmetic uses single-precision float (the F746 has an SFPU);
     * double operations fall back to software (the F746 has no double-precision
...
int main(void)
{
    ...
    printf("\r\n=== blink_hello on STM32F746ZG @ %lu Hz ===\r\n",
