# CoreMark 1.0 @ 216 MHz - STM32F746ZG (NUCLEO-F746ZG)
...
sources build with **GNU arm-none-eabi-gcc**, **Keil Arm Compiler 6
(armclang)** or **ST Arm clang** (starm-clang), selected at configure time.
The port uses the HAL SysTick 1 kHz tick (`HAL_GetTick()`) for CORE_TICKS, so
it works identically on all three compilers.

## Results (measured on hardware, 216 MHz, single-precision FPU, I/D caches on)

| Toolchain     | Flags                                                 | Iterations/s | Total time |
| ------------- | ------------------------------------------------------ | ------------ | ---------- |
| GCC 15.3.1    | `-Ofast -ffp-contract=fast -funroll-all-loops`         | 926.23       | 26.99 s    |
| GCC + LTO     | same + `-flto`                                         | 865.53       | 28.88 s    |
| ARMCLANG 6.24 | `-Ofast -ffp-contract=fast -funroll-all-loops`         | 914.91       | 27.33 s    |
| ARMCLANG 6.24 | `-Omax -fno-lto`                                       | **1061.93**  | **23.54 s**|
| ST Arm clang  | `-Ofast -ffp-contract=fast -funroll-all-loops`         | 790.89       | 31.61 s    |

Measured on hardware: every configuration printed **`Correct operation
validated.`** with the expected CRCs (seedcrc 0xe9f5, crcfinal 0xcc42). GCC
measured 926.34/926.23 and armclang -Omax 1061.98/1061.93 on consecutive
runs; the table records the last complete run.

Scores are within ~1.6% of the nucleo-f722 (same Cortex-M7 core @ 216 MHz,
heap in on-chip SRAM). The best measured 1061.93 is close to ST's published
~1073 CoreMark for a Cortex-M7 @ 216 MHz.

## Most aggressive flags

- **ARMCLANG (Keil AC6): `-Omax -fno-lto`** - **1061.93 iterations/s**, +16%
  over the `-Ofast`-class flags (armclang is flat across -O3/-Ofast/unroll at
  ~915). Bare `-Omax` makes armclang emit **LLVM LTO objects** that GNU ld
...
- **GCC:** `-Ofast -ffp-contract=fast -funroll-all-loops` (the default) →
  926.23 it/s; `-funroll-all-loops` beats the `-funroll-loops` build
  (924.35 on the f722) by a hair. **Do not add `-flto`** - it *loses* ~6.5%
  here (865.53).
- **ST Arm clang:** 790.89 it/s at the default flags; flat across the tuning
  flags tried.
...
# armclang at -Omax (best measured: 1061.93 it/s):
