...
static void ADC_Print(void)
{
    ...
    printf("ADC: VREFINT raw=%lu cal=%u -> VDDA=%.0f mV | Temp=%.1f C | VBAT=%.2f V\r\n",
           (unsigned long)vrefint_raw, (unsigned)vrefint_cal, vdda_mv,
           temp_c, vbat_mv / 1000.0f);
}
...
int main(void)
{
    ...
    {
        LED_ToggleAll();
        printf("LED LD1(PJ13):%s LD2(PJ5):%s LD3(PA12):%s @ %lu Hz\r\n",
               HAL_GPIO_ReadPin(LED1_PORT, LED1_PIN) == GPIO_PIN_SET ? "ON" : "OFF",
               HAL_GPIO_ReadPin(LED2_PORT, LED2_PIN) == GPIO_PIN_SET ? "ON" : "OFF",
               HAL_GPIO_ReadPin(LED3_PORT, LED3_PIN) == GPIO_PIN_SET ? "ON" : "OFF",
               (unsigned long)SystemCoreClock);
        ADC_Print();
        HAL_Delay(1000);
    }
